...
export default function AdventurePage() {
    ...
    // --- State ---
    const [hasMounted, setHasMounted] = useState(false); // 安全装置
    ...
    // 2. データ読み込み
    useEffect(() => {
        if (!params.id || !hasMounted) return;
    ...
    // 3. 位置追跡ロジック
    useEffect(() => {
        if (!currentItem || !hasMounted || typeof window === "undefined") return;

        const watchId = navigator.geolocation.watchPosition(
            (pos) => {
                // コールバック内でもデータ存在チェックを徹底
        ...
        return (
            <div className="flex flex-col h-screen bg-gradient-to-br from-[#F06292] to-[#FF8A65] text-white items-center justify-center p-8 text-center">
                <div className="bg-white/20 backdrop-blur-3xl rounded-[3rem] p-10 shadow-2xl border border-white/30 animate-fade-in-up">
                    ...
                    <h1 className="text-4xl font-black mb-2 italic tracking-tighter">COMPLETED!</h1>
                    <p className="opacity-90 font-bold mb-8">すべてのアイテムを見つけた！</p>
    ...
    return (
        <div className="flex flex-col h-screen relative overflow-hidden bg-white">
            ...
            <main className="flex-1 flex flex-col items-center justify-center relative z-10 px-6">
                ...
                <div className="mt-8 bg-black/70 backdrop-blur-md px-6 py-2 rounded-full border border-white/10">
