            ...
            const watchId = navigator.geolocation.watchPosition((pos) => {
                ...
                geocoder.geocode({ location: newLoc }, (res, status) => {
                    if (status === "OK" && res?.[0]) {
                        ...
                        const locality = comp.find(c => c.types.includes("locality"))?.long_name || comp.find(c => c.types.includes("administrative_area_level_2"))?.long_name || "";
        ...
        <div className="relative h-screen bg-white overflow-hidden flex flex-col">
            ...
            <header className="relative z-10 p-6 pt-16 text-left">
                <div className="bg-black/90 backdrop-blur-xl rounded-[2.5rem] p-6 shadow-2xl text-white">
                    <p className="flex items-start gap-1 mb-1 text-[#F37343] font-black text-[10px] uppercase tracking-widest"><MapPin size={10} /> <span>{currentAreaName}</span></p>
