...
import {
    CheckCircle2, Loader2, Flag, ArrowUp, MessageSquare, Send, ChevronLeft, ChevronRight, Beaker, Circle
} from "lucide-react";
        ...
        <div className="h-screen bg-white flex flex-col relative overflow-hidden text-black font-sans">
            ...
            <header className="p-8 pt-14 flex justify-between items-baseline z-20">
                <div className="flex flex-col gap-2">
                    ...
                    {/* ★進捗アイコン列：プラン内の位置（インデックス）に基づいて塗りつぶし */}
            ...
            {!isMissionComplete && (
                <div className="p-8 pb-12 z-20 flex flex-col items-center">
                    <div className="flex items-center gap-6 mb-10">
                        <button onClick={() => { if (uncollectedItems.length > 1) { const idx = uncollectedItems.findIndex((i: any) => i.id === activeTarget?.id); setManualTargetId(uncollectedItems[(idx - 1 + uncollectedItems.length) % uncollectedItems.length].id); } }} className="p-2 text-gray-200 active:text-pink-500"><ChevronLeft size={24} /></button>
                        ...
                        <button onClick={() => { if (uncollectedItems.length > 1) { const idx = uncollectedItems.findIndex((i: any) => i.id === activeTarget?.id); setManualTargetId(uncollectedItems[(idx + 1) % uncollectedItems.length].id); } }} className="p-2 text-gray-200 active:text-pink-500"><ChevronRight size={24} /></button>
                    </div>
                    ...
                    <div className="mt-8 flex gap-4 opacity-10 hover:opacity-100 transition-opacity">
                        <button onClick={() => setDistanceToTarget(48)} className="flex items-center gap-1 text-[8px] font-bold border border-gray-100 px-3 py-2 rounded-full text-gray-300"><Beaker size={10} /> 近接</button>
                        <button onClick={() => { if (activeTarget) handleAcquireItem(activeTarget); }} className="flex items-center gap-1 text-[8px] font-bold border border-gray-100 px-3 py-2 rounded-full text-gray-300"><CheckCircle2 size={10} /> 獲得</button>
                    </div>
                </div>
            )}
