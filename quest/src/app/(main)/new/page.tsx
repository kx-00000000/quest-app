            ...
            const result = await new Promise<any>((resolve) => {
                geocoder.geocode({ location: point }, (res, status) => {
                    if (status === "OK" && res?.[0]) {
                        ...
                        // ★ 改良：市区町村があり、かつ海上（natural_feature単体）ではないことを確認
                        ...
                        if (pref && locality && country) {
                            // ★ 解決：ランダムな座標ではなく、Geocoderが返した「正式な陸地の座標」を採用する
                            const landPos = {
                ...
                <>
                    ...
                    <div className="mt-auto relative z-10 px-4 mb-4"><div className="bg-white/80 backdrop-blur-xl rounded-[3rem] p-6 shadow-2xl border border-white space-y-5">
                        ...
                        <div className="space-y-4 px-1">
                            ...
                            <div className="space-y-1"><div className="flex justify-between text-[10px] font-black text-[#F37343] uppercase tracking-widest"><span>Items Count</span><span>{itemCount}</span></div><input type="range" min="1" max="7" step="1" value={itemCount} onChange={(e) => { setItemCount(parseInt(e.target.value)); resetReadyStatus(); }} className="w-full h-3 bg-gray-100 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:bg-[#F37343] [&::-webkit-slider-thumb]:rounded-full" /></div>
                        </div>
                        <button onClick={handleAction} disabled={isCreating} className={`w-full py-4 rounded-[2rem] font-black flex items-center justify-center gap-2 transition-all duration-500 shadow-lg uppercase active:scale-95 ${isBriefingReady ? "bg-gradient-to-r from-[#F37343] to-orange-400 text-white" : "bg-gray-900 text-white"}`}>{isCreating ? <Loader2 className="animate-spin" /> : isBriefingReady ? <><Play size={16} fill="currentColor" /> START BRIEFING</> : "CREATE QUEST"}</button>
                    </div></div>
                </>
            )}
            ...
            {isFinalOverview && (
                <div className="absolute inset-0 z-[3000] flex items-center justify-center p-6 bg-black/40 backdrop-blur-[2px] animate-in fade-in duration-700">
                    <div className="bg-white/95 backdrop-blur-xl rounded-[2.5rem] p-8 w-full max-w-sm space-y-6 shadow-2xl relative overflow-hidden text-center border border-white/50">
                        ...
                        <div className="space-y-2 py-4 px-2">{briefingItems.map((item, idx) => (<div key={idx} className="flex items-center gap-3 text-left"><span className="flex-none w-5 h-5 bg-[#F37343] text-white text-[10px] font-black flex items-center justify-center rounded-full shadow-sm">{idx + 1}</span><span className="text-[12px] font-bold text-gray-700 uppercase truncate flex-1 tracking-tight">{item.addressName}</span></div>))}</div>
                        <button onClick={() => router.push("/plan")} className="w-full py-4 bg-gray-900 text-white rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-3 active:scale-95 transition-transform"><Play size={14} fill="currentColor" /><span>Confirm Quest</span></button>
                    </div>
                </div>
            )}
        </div>
