        ...
        <div className="flex flex-col h-full min-h-screen pb-20 relative overflow-hidden bg-white">
                ...
                <div className="mt-auto relative z-10 px-4 mb-4">
                    <div className="bg-white/80 backdrop-blur-xl rounded-[3rem] p-6 shadow-2xl border border-white space-y-5">
                        <input
                            ...
                            className="w-full bg-black/5 p-4 rounded-2xl border-none outline-none text-gray-800 font-black text-center"
                        />
                        <div className="space-y-4 px-1">
                            <div className="flex justify-between text-[10px] font-black text-pink-500 uppercase">
                                <span>Radius</span>
            ...
            {showConfirm && (
                <div className="absolute inset-0 z-[2000] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white rounded-[3rem] p-8 shadow-2xl w-full max-w-sm text-center space-y-6">
                        <CheckCircle2 size={32} className="text-pink-500 mx-auto" />
                        <h3 className="text-xl font-black">QUEST READY</h3>
                        <button
                            onClick={() => { setShowConfirm(false); setIsBriefingActive(true); }}
                            className="w-full py-4 bg-gray-900 text-white rounded-[2rem] font-black flex items-center justify-center gap-2 shadow-xl"
                        >
            ...
            {isFinalOverview && (
                <div className="absolute inset-0 z-[3000] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md">
                    <div className="bg-white rounded-[3rem] p-8 w-full max-w-sm text-center space-y-6 shadow-2xl relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-pink-500 to-orange-400" />

                        <header className="space-y-1">
                            <p className="text-[9px] font-black text-pink-500 uppercase tracking-[0.3em]">Discovery Report</p>
                            <p className="text-[8px] font-bold text-gray-300 uppercase tracking-widest italic">All waypoints identified</p>
                        </header>

                        <div className="space-y-1.5 py-3 px-5 bg-gray-50 rounded-[2rem] border border-gray-100">
                            {briefingItems.map((item, idx) => (
                                <div key={idx} className="flex items-baseline gap-3">
                                    <span className="text-[10px] font-black text-pink-500 tabular-nums">#{idx + 1}</span>
                                    <span className="text-sm font-black text-gray-900 uppercase tracking-tight truncate text-left flex-1">
                                        {item.addressName}
                                    </span>
                                </div>
                            ))}
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                <Target size={16} className="text-gray-300 mx-auto mb-1" />
                                <p className="text-[8px] font-bold text-gray-400 uppercase">Waypoints</p>
                                <p className="text-lg font-black">{briefingItems.length}</p>
                            </div>
                            <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                <Navigation size={16} className="text-gray-300 mx-auto mb-1" />
                                <p className="text-[8px] font-bold text-gray-400 uppercase">Range</p>
                                <p className="text-lg font-black">{formatDistance(radius)}</p>
                            </div>
                        </div>
