...
// 地図のアイコンやズームを管理するサブコンポーネント
function MapEffect({ radiusInKm, center, color }: { radiusInKm?: number, center?: { lat: number, lng: number } | null, color: string }) {
    const map = useMap();

    useEffect(() => {
        // ブラウザ環境であることを確認してから実行
...
export default function MapComponent({ radiusInKm, items, path, center, userLocation, themeColor }: any) {
    ...
    if (!isClient || !L) return <div className="h-full w-full bg-gray-100" />;
    ...
    return (
        <MapContainer center={mapCenter} zoom={13} style={{ height: "100%", width: "100%" }} zoomControl={false} attributionControl={false}>
            ...
            <MapEffect radiusInKm={radiusInKm} center={userLocation || center} color={displayColor} />
