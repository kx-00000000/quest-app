...
export default function MissionBriefing({ items, onStateChange, onComplete }: { items: any[], onStateChange: (val: boolean) => void, onComplete: () => void }) {
    ...
    return (
        <div className="absolute inset-0 z-[1000] pointer-events-none p-4 flex flex-col items-center">
            {!isFinalOverview && currentIndex !== -1 && (
                <div className="mt-8 bg-gray-900/95 backdrop-blur-2xl px-5 py-3 rounded-full shadow-2xl border border-white/10 animate-in fade-in slide-in-from-top-4">
            ...
            {isFinalOverview && (
                <div className="mt-auto mb-28 w-full max-w-[320px] bg-gray-900/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl border border-white/10 p-6 animate-in slide-in-from-bottom-10">
                    <h2 className="text-[9px] font-black text-pink-500 uppercase tracking-[0.3em] mb-4 text-center">Discovery Report</h2>
